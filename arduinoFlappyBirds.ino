...
#define BUZZER 13 // Define pin of the Passive buzzer
#define NOTE_SUSTAIN 50 // Define melody note delay in ms
...
bool hasRun = false;    // Condition for noise/buzzer at start/end of game
...
void checkCollision()
{
  ...
  { // if the bird has hit a pipe, 
    ...
    for(int i=0;i<80;i++) // Activate buzzer frequency e.g; play noise
    {
  ...
  { // If the bird hits a coin
    ...
    {
      digitalWrite(BUZZER,HIGH);
      delayMicroseconds(800);//wait for .5ms
      digitalWrite(BUZZER,LOW);
      delayMicroseconds(800);//wait for .5ms
    }
    for(int i = 0;i < 30;i++) // Activate second frequency e.g; play noise
    {
      digitalWrite(BUZZER,HIGH);
      delayMicroseconds(500);//wait for .5ms
      digitalWrite(BUZZER,LOW);
      delayMicroseconds(500);//wait for .5ms
    }
  }
...
void playHighScoreBuzz()
{
  highScore = true;

  for(uint8_t nLoop = 0;nLoop < 2;nLoop ++)
      { // TODO: Make into array of notes
        tone(BUZZER,NOTE_A5);
        delay(NOTE_SUSTAIN);
        tone(BUZZER,NOTE_B5);
        delay(NOTE_SUSTAIN);
